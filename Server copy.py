...
class Server:
    ...
    def run(self):
        try:
            ...
            #     pass
            Speaker.speak("我在")
            command_queue = queue.Queue()  # 创建一个指令队列
            while True:
                ...
                if msg_type == 'shutdown':
                    ...
                    player = MusicPlayer('消愁', command_queue)
                    # 启动播放线程
            ...
            # result={}
            # recognize_thread = threading.Thread(target=record_and_recognize, args=(result))
            # recognize_thread.start()
            # player = MusicPlayer('消愁')
            # player_thread = threading.Thread(target=player.play,args=(result))
            # player_thread.start()
